...
/*
 * Script 8 - All Deoxys Forms (Ultra Moon)
 * ---------------------------------------------------------------------------
 * Deoxys (#386) has 4 formes:
 *   0 Normal, 1 Attack, 2 Defense, 3 Speed.
 * All shiny copies first, then all non-shiny copies (Deoxys can be shiny).
 *
 *   Trainer (OT) : Cole (Female)   IVs: 31   TID/SID: random (one set/run)
 *
 * Legality note: Deoxys is a transfer-only Mythical in Gen VII; its forme is
 * changed in-game by the meteorite shards. Verify in PKHeX.
 * ---------------------------------------------------------------------------
 */
...
int main(int argc, char** argv)
{
    ...
    gui_warn("All 4 Deoxys formes injected!\nOT: Cole (Female)\nShiny copies first, then non-shiny.");
    return 0;
}
